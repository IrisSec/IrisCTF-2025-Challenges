...
func main() {
	...
	err = http.ListenAndServe(":1337", rawMux(mux))
	if err != nil {
...
func isLoggedIn(w http.ResponseWriter, r *http.Request) (bool, error) {
	...
	data, err := base64.StdEncoding.DecodeString(authCookie.Value)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		return false, err
	}
...
func getpasswords(w http.ResponseWriter, r *http.Request) {
	...
	query, err := DB.Query("SELECT URL, Title, Username, Password FROM passwords")
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		fmt.Println(err)
		return
	...
	for query.Next() {
		var url, title, username, password string

		if err := query.Scan(&url, &title, &username, &password); err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			fmt.Println(err)
			return
		}
	...
	err = json.NewEncoder(w).Encode(passwords)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		fmt.Println(err)
		return
...
func homepage(w http.ResponseWriter, r *http.Request) {
	if loggedIn, _ := isLoggedIn(w, r); loggedIn {
		http.Redirect(w, r, "/passwords", http.StatusMovedPermanently)
		return
	}
