                          ...
                          const uint8_t *m, size_t mlen, const uint8_t *sk)
{
    ...
    for (i = 0; i < SPX_D; i++) {
        ...
        merkle_sign(sig, root, &ctx, wots_addr, tree_addr, idx_leaf, i);
                       ...
                       const uint8_t *m, size_t mlen, const uint8_t *pk)
{
    ...
    if((tree & 0xFFFFULL) != 0x1337ULL) { // Only cooler signatures allowed
        return -2;
    }
                     ...
                     const unsigned char *pk)
{
    ...
    *mlen = smlen - SPX_BYTES;
    ...
    if ((ret_val = crypto_sign_verify(sm, SPX_BYTES, sm + SPX_BYTES, (size_t)*mlen, pk))) {
        memset(m, 0, smlen);
        *mlen = 0;
        return ret_val;
    }
